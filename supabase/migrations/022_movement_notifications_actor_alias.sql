...
BEGIN
  IF NEW.type = 'income' THEN
    ...
    v_body := 'Registraste un ingreso de ' || NEW.amount::TEXT || ' en ' || COALESCE(NEW.category, 'General') || '.';
  ELSIF NEW.type = 'expense' THEN
    v_title := 'Gasto registrado';
    v_body := 'Registraste un gasto de ' || NEW.amount::TEXT || ' en ' || COALESCE(NEW.category, 'General') || '.';
  ...
  ELSE
    ...
    v_body := 'Registraste un movimiento de ' || NEW.amount::TEXT || '.';
  END IF;
  ...
  IF NEW.is_couple THEN
    ...
    IF v_partner_id IS NOT NULL THEN
      ...
      IF NEW.type = 'income' THEN
        ...
        v_partner_body := v_actor_label || ' registro un ingreso de ' || NEW.amount::TEXT || ' en ' || COALESCE(NEW.category, 'General') || '.';
      ELSIF NEW.type = 'expense' THEN
        v_partner_title := v_actor_label || ' registro un gasto';
        v_partner_body := v_actor_label || ' registro un gasto de ' || NEW.amount::TEXT || ' en ' || COALESCE(NEW.category, 'General') || '.';
      ELSIF NEW.type = 'transfer_to_joint' THEN
      ...
      ELSE
        ...
        v_partner_body := v_actor_label || ' registro un movimiento de ' || NEW.amount::TEXT || '.';
      END IF;
